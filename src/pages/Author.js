...
export default function Author() {
  return (
    ...
    <p className={Styles.Link}><a href="https://www.ieee.org/conferences/publishing/templates.html" target="_blank" rel="noopener noreferrer">Click here to download paper templates in word and LaTeX.</a></p>
    <div className={Styles.subHeading}>
        To know more about how to create papers for IEEE IES conferences, please visit:
    </div>
    <p className={Styles.Link}><a href="http://www.ieee-ies.org/conferences/paper-format-for-conferences" target="_blank" rel="noopener noreferrer">Click here for more information.</a></p>
    <div className={Styles.subHeading}>Instructions for Authors</div>
    <p className={Styles.normalText}>Prospective authors are invited to electronically submit full regular papers of their work. English is the working language of the conference. Papers are limited to six (6) pages. Up to two (2) additional pages can be accepted, but additional page fees must be paid at registration.</p>
    <p className={Styles.normalText}>Authors must adhere to the IEEE Conference written paper format. The author must submit the draft version of the paper for review before the due submission deadlines (see dates above). These are submitted through the IES Submit system, which is entered from the conference website. IEEE is very strict about the requirements for PDF files for inclusion in the IEEE Xplore® Digital Library. We strongly recommend to use the set of templates in MS Word and LaTeX format provided by IEEE. Letter page size is preferred, and A4 can also be used. Please use them to create your paper, but don’t modify the style or the format under any circumstances</p>
    <p className={Styles.normalText}>Submit only original work, not previously published or copyrighted. Prospective authors are expected to submit only their own original works. The Conference will be using the CrossCheck automated screening system to help verify originality of papers. Submitted works may be compared to over 20 million articles in databases worldwide. Papers that violate IEEE’s publication principles may be summarily rejected. If the violation is deemed severe, then disciplinary actions may also be taken by IEEE.</p>
